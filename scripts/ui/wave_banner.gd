...
func show_wave_banner(wave: int, is_boss: bool = false) -> void:
	...
	tween.tween_property(banner_label, "modulate:a", 1.0, 0.3).set_ease(Tween.EASE_OUT)
	await get_tree().create_timer(2.0).timeout
	var fade := get_tree().create_tween()
	fade.tween_property(banner_label, "modulate:a", 0.0, 0.4)
	fade.tween_callback(_on_banner_finished)
